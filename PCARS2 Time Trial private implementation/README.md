...
Valid completed laps are also appended to `src/Pcars2Collector/logs/laps.log`. Invalidated laps are not written there; the frequent telemetry polling logs remain in the console.

Tests are under `tests/Pcars2Collector.Tests` and cover lap completion transitions and invalidated laps.

## Next steps

- Add an HTTPS event sink to the Fedora backend.
- Persist a local outbox so temporary network failures do not lose laps.
